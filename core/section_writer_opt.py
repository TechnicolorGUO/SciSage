    ...
    while retries <= rag_config.max_retries:
        try:
            async with global_semaphores.rag_semaphore:
                session = await rag_config.session
                logger.info(
                ...
                logger.debug(
                    f"[{query_summary}] RAG payload: {json.dumps(payload, indent=2)}"
                )

                async with session.post(rag_config.url, json=payload) as response:
                    # Process response
                    if response.status != 200:
                        error_detail = await response.text()
                        logger.warning(
                            f"RAG service returned HTTP {response.status}: {error_detail}"
                        )
                        ...
                        raise HTTPException(
                            ...
                            detail=f"RAG service returned error: {error_detail}",
                        )

                    response_data = await response.json()
                    if response_data.get("status") != 200:
                        logger.warning(
                            f"RAG service returned status {response_data.get('status')}"
                        )
                        if retries < rag_config.max_retries:
                            retries += 1
                            await asyncio.sleep(rag_config.retry_delay)
                        ...
                        raise HTTPException(
                            status_code=500,
                            detail=f"RAG service returned error: {response_data}",
                        )

                    # Extract data from successful response
                    generated_content = response_data.get("output", "")
                    ...
                    report_index_list = convert_ctx_to_entities(response_data)
                    main_figure_data = response_data.get("main_figure_base64", "")
                    main_figure_caption = response_data.get("main_figure_caption", "")
    ...
    if failed_queries and len(successful_results) > 0:
        ...
        for query, error, key_point in failed_queries:
            ...
            fallback_result = RagResult(
                ...
                section_index=state.section_index,
                parent_section=state.parent_section,
                section_text=f"Content for {key_point} could not be generated. Please refer to other sections or try again.",
                main_figure_data="",
                main_figure_caption="",
...
async def section_writer_async(
    ...
    try:
        ...
        state = SectionWriterState(
            ...
            rag_model=params.get("rag_model", "openscholar"),
        )

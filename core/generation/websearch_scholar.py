...
from generation.utils import get_md5, keep_letters
import arxiv
import json
import random
import re
        ...
        for article in records["PubmedArticle"]:
            try:
                ...
                try:
                    ...
                    if (
                        "PubmedData" in article
                        and "ReferenceList" in article["PubmedData"]
                    ):
                        for ref_list in article["PubmedData"]["ReferenceList"]:
                            for ref in ref_list["Reference"]:
                                ...
                                ref_info = {
                                    "title": (
                                        str(ref.get("ArticleTitle", ""))
                                        if ref.get("ArticleTitle")
                                        else ""
                                    ),
                                    "citation": (
                    ...
                    if not references and "pmc" in identifiers:
                        ...
                        try:
                            ...
                            if ref_results and len(ref_results[0]["LinkSetDb"]) > 0:
                                ref_pmids = [
                                    link["Id"]
                                    for link in ref_results[0]["LinkSetDb"][0]["Link"]
                                ]

                                # Get basic info for each reference
                                if ref_pmids:
                                    ref_handle = Entrez.esummary(
                                        db="pubmed", id=",".join(ref_pmids)
                                    )
                            ...
                            logger.warning(
                                f"Error getting references from PMC: {str(e)}"
                            )
                except Exception as e:
            ...
            if response.status_code == 200:
                ...
                for work in data["results"]:
                    try:
                        ...
                        if best_oa_location and best_oa_location.get("source"):
                            oa_source = best_oa_location["source"]
                            if not venue_info:  # 如果primary_location没有，从这里取
                                venue_info = oa_source.get("display_name", "")
                                journal_info["name"] = venue_info

                        # 从 locations 获取所有可能的发布位置
                        ...
                        # 构建 journal_ref 字符串
                        if venue_info:
                            journal_ref = venue_info
                            if publication_year:
                                journal_ref += f" ({publication_year})"

                            # 添加卷号和期号信息（如果有的话）
                            biblio = work.get("biblio", {})
                            if biblio:
                                if biblio.get("volume"):
                                    journal_ref += f", Vol. {biblio['volume']}"
                                if biblio.get("issue"):
                                    journal_ref += f", No. {biblio['issue']}"
                                if biblio.get("first_page") and biblio.get("last_page"):
                                    journal_ref += f", pp. {biblio['first_page']}-{biblio['last_page']}"
                                elif biblio.get("first_page"):
                                    journal_ref += f", p. {biblio['first_page']}"
                        ...
                        logger.info(f"Venue: {venue_info}")
                        logger.info(f"Journal Ref: {journal_ref}")
                        logger.info(f"DOI: {doi}")
                        logger.info(f"Type: {type_info}")

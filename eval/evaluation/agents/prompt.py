...
* **1**: No clear logic; disconnected and confusing structure.
* **2**: Weak flow; several disordered or unreasonable arrangements.
* **3**: Generally reasonable; some redundant sections or rough transitions.
* **4**: Good structure; mostly smooth and well-organized with minor rigidity.
* **5**: Excellent structure; logically tight with smooth, natural transitions and no redundancy.
...
* `"structure_score"`: an integer from 1 to 5
* `"reasons"`: a list of brief issues if the score is less than 5; otherwise leave it empty.
...
Only return the JSON. Do not include any extra explanation or commentary.
"""
...
Final Score:
<SCORE>(X+Y+Z)/3=...</SCORE>
(Example: <SCORE>(2.5+7+5.1)/3=4.87</SCORE>; scores can include two decimal place, do not contain any other text)

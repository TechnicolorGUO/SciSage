...
def evaluate(jsonl_file, eval_model, infer_type, skip_titles, skip_line_number=0):
    def eval_single_survey(judge, survey, topic, outline, papers):
        ...
        result = {
            ...
            "filtered_scores_lang": str(dimension_scores["filtered_scores_lang"]),
            "filtered_scores_crit": str(dimension_scores["filtered_scores_crit"]),
            "structure": float(scores[0])* 20,
            "relevance": float(scores[1]) * 20,
            "outlinse_score": float(outlinse_score),
            "compression_ratio": float(fact_stats.get("compression_ratio",0)),
        }
        return result
    ...
    with open(jsonl_file, "r") as f:
        with futures.ThreadPoolExecutor(5) as executor:
            ...
            for line_number, line in enumerate(f, start=1):
                ...
                print(f"""query: {data["title"]}""",flush=True)
                topic = data["title"]
                papers = data["papers"]
